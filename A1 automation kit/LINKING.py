...
# Function to remove lines containing "CENTRAL BANK OF INDIA" and the next 3 lines
...
# Updated function to keep only lines containing "CIF LINKING" or "CIF DELINKING"
def filter_cif_linking_delinking(lines):
    return [line for line in lines if "CIF LINKING" in line or "CIF DELINKING" in line]
...
def select_files():
    ...
    input_files = filedialog.askopenfilenames(
        title="select BR_Report_on_CIF_Linking_and_DeLinking.txt",
...
output_file = os.path.join(script_dir, "CIFlinking.txt")
...
for file_path in input_files:
    ...
    with open(file_path, 'r') as file:
        ...
        # Filter lines that contain "CIF LINKING" or "CIF DELINKING"
        lines_to_write = filter_cif_linking_delinking(lines[11:])  # Skip the first 11 lines
        print(f"{filename}: Writing {len(lines_to_write)} filtered lines.")  # Log number of lines being written
