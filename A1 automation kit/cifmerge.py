...
# Filter function to include only lines with "MOBILE NO" 
def filter_line(line):
    return "MOBILE NO" in line
...
def select_files():
    ...
    input_files = filedialog.askopenfilenames(
        title="select BR_Report_on_Changes_in_CIF_And_Accounts.txt",
...
output_file = os.path.join(script_dir, "CIFMOB.txt")
...
for file_path in input_files:
    ...
    with open(file_path, 'r') as file:
        ...
        # Skip the first 10 lines and filter to include only lines with "MOBILE NO"
        lines_to_write = [line for line in lines[10:] if filter_line(line)]
