...
with open('Intrestrate.txt', 'r') as file:
...
if not text_data.strip():  # Check if the text data is empty or contains only whitespace
    ...
    sheet = wb.sheets['INTR']  # Select the sheet you want
...
# text_file_path = 'Intrestrate.txt'
...
wb.macro('interestvar')()  # Uncomment this line and replace 'ACTV' with the macro name if needed
...
# Step 6.1: deleting the equal interest
for i in range(last_row, 0, -1):
    if sheet.range(f'E{i}').value == sheet.range(f'F{i}').value:
