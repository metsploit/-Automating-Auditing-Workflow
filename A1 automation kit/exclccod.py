...
with open('CCODDLTL.txt', 'r') as file:
...
if not text_data.strip():  # Check if the text data is empty or contains only whitespace
    ...
    sheet = wb.sheets['TINKI']  # Select the sheet you want
...
wb.macro('TINKI')()  # Uncomment this line and replace 'ACTV' with the macro name if needed
...
# Loop through each row in reverse to delete rows where E < D
for i in range(last_row, 0, -1):
    if sheet.range(f'F{i}').value < sheet.range(f'E{i}').value:
        sheet.range(f'{i}:{i}').delete()  # Delete the entire row
