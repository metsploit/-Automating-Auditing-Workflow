...
def remove_central_bank_lines(content):
    ...
    for line in lines:
        ...
        if "CENTRAL BANK OF INDIA" in line:
            skip_count = 4  # Skip the next 9 lines as well
        elif "AC-TOTAL" in line:
            continue  # Skip lines containing "AC-TOTAL"
        elif "BR-TOTAL" in line:
            continue  # Skip lines containing "BR-TOTAL"
        else:
...
def select_files():
    ...
    input_files = filedialog.askopenfilenames(
        title="select monthly report Audit_BGL_accounts_age_wise_break_up_gend0805.txt",
...
output_file = os.path.join(script_dir, "HOUSEKEEPING.txt")
...
# Write the modified content bac# Add 8 spaces at the start of the first line
with open(output_file, 'r') as output:
    final_content = output.readlines()

final_content[0] = ' ' * 6 + final_content[0]  # Add 8 spaces at the beginning of the first linek to the file
with open(output_file, 'w') as output:
